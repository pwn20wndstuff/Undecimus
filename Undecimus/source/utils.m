    ...
    switch (exploit) {
        ...
        case v1ntex_exploit: {
            ...
            for (NSString *string in list) {
                vm_size_t vm_size = 0;
                if (kernelVersionContains(string.UTF8String) && host_page_size(mach_host_self(), &vm_size) == ERR_SUCCESS && vm_size == 0x1000) {
                    return true;
                }
            }
            break;
...
bool jailbreakSupported() {
    ...
    supportsExploit(voucher_swap_exploit) || supportsExploit(v1ntex_exploit);
}
