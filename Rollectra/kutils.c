...
uint64_t task_self_addr() {
  if (cached_task_self_addr == 0) {
    cached_task_self_addr = getAddressOfPort(getpid(), mach_task_self());
...
uint64_t current_thread() {
  uint64_t thread_port = getAddressOfPort(getpid(), mach_thread_self());
...
mach_port_t fake_host_priv() {
  ...
  // locate the port
  uint64_t port_addr = getAddressOfPort(getpid(), port);
